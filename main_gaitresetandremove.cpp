...
int main(int argc, char **argv){
	...
	vector<Filament> filaments_copy;
	...
	if(myrank == 0) {
		if(checkpointing) {
			...
			if (SaveExtendedForcesData) {
				save_data_column_names_to_forces_file(SimulationForceDataName, filaments);
				save_force_data_to_file(SimulationForceDataName,filaments,NULL,NULL,0);
			}
			#if GaitReset
				...
				save_data_column_names_to_swimmer_velocity_file(SimulationGaitResetAndRemoveName, filaments);
				save_swimmer_velocity_data_to_file(SimulationGaitResetAndRemoveName,filaments,NULL,NULL,NULL,0);
			#endif
		}
		else {
	...
	int Np_copy = Np;
	MPI_Bcast(&Np_copy, 1, MPI_INT, 0, MPI_COMM_WORLD);
	...
	for (int nt=nt_start; nt<=timesteps; ++nt) {
		...
		// gait_reset_i = 1: fake step; no collisions at all and no filaments
		// gait_reset_i = 0: fake step; no collisions at all
		// for (int gait_reset_i=2; gait_reset_i<=2; ++gait_reset_i)
		for (int gait_reset_i=0; gait_reset_i<=2; ++gait_reset_i)
		{

			if(gait_reset_i == 1) {
				Np = 15;
				filaments_copy = filaments;
				filaments.resize(1);
			} else if (gait_reset_i == 2) {
				Np = Np_copy;
				filaments = filaments_copy;
			}
			...
			if(myrank == 0) {
				...
				if(gait_reset_i == 2) {
					...
					if (SaveExtendedForcesData) { // Saves every timestep. You may want to change this.
						save_force_data_to_file(SimulationForceDataName,
						                        filaments,
						                        fcm.F, fcm.T,
						                        nt);
					}
					...
					save_swimmer_velocity_data_to_file(SimulationGaitResetAndRemoveName,
					                                   filaments,
					                                   fcm.V, fcm.W, fcm.F,
					                                   nt);
				} else { // gait_reset_i == 0
