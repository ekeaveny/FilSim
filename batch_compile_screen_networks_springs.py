...
# Swimming
if SwimProblem:
    SimName0 = 'SwimNodes'  # 'ConvCheckBDFb'
    ...
    Nworm = 60 # Maximum length
    Nworm_swimmer = 15

    ConnectedNodesInitialisation = False
    ConnectedNodesInitialisationAllowOverlap = True
    ConnectedNodesNumSwimmingFilaments = 1
    ConnectedNodesNumNodes = 280 #
    ...
    ConnectedNodesSpacingAwayFromNode = 2.0
    ConnectedNodesConnectionProbability = 0.17 #

    StepsPerPeriod = 4000 # 4500 # 7500 #45000 #600

    initialFrictionSteps = 4*StepsPerPeriod # 0  # 1000

    RandomMessNumNetworkFilaments = [0]  # [180, 300, 420]
    #Nsw = [0 + i for i in RandomMessNumNetworkFilaments]  # 0 swimmers
    # Nsw = [1 + i for i in RandomMessNumNetworkFilaments] # 1 swimmer
    Nsw = [2750] # [1200,1200,2000,2700] Maximum number of filaments allowed (in spring node mode)
    ...
    # Spring data
    EnableSpringLinks = True
    NetworkSpringConstant = [0,0.01,0.1]#[0,0.01,0.1]  # k in the spring force k*(x-L)
    NetworkSpringNaturalLength = 2.2  # L in the spring force k*(x-L)

    NetworkSpringConnectionRadius = [15 * 2.2]  # Deprecated
    ProbabilityOfCreatingANetworkSpringLink = 1  # Not used. Form a spring link with this probability
        ...
        for k, Bnumber_i in enumerate(Bnumber):
            for j, NPTS_i in enumerate(NPTS):
                for randSeed_i in randSeed:
                    for NetworkSpringConstant_i in NetworkSpringConstant:
                        for NetworkSpringConnectionRadius_i in NetworkSpringConnectionRadius:

                            Barrier_FS_over_2a_i = Barrier_FS_over_2a[k]
                            #NetworkSpringConstant_i = Kap_NetworkFilaments_i * NetworkSpringConstantFactor_i

                            if SedimentationProblem:
                                SimName = SimName0 + "-N" + str(Nsw_i) + "-B" + str(Bnumber_i) + "-TallBox" + "-NPTSx" + str(NPTS_i[0]) + "y" + str(NPTS_i[1]) + "z" + str(NPTS_i[2])
                            if SwimProblem:
                                SimName = (SimName0
                                           #+ "-Nnetfil" + str(RandomMessNumNetworkFilaments_i)
                                           + "-Kap" + str(Kap_NetworkFilaments_i)
                                           + "-NPTSx" + str(NPTS_i[0]) + "y" + str(NPTS_i[1]) + "z" + str(NPTS_i[2])
                                           + "-randSeed" + str(randSeed_i)
                                           + "-springk" + str(NetworkSpringConstant_i).replace(".","p")
                                           #+ "-springRconn" + str(NetworkSpringConnectionRadius_i)
                                           + "-nodes" + str(ConnectedNodesNumNodes)
                                           )
                            ...
                            with open('config.hpp', 'r') as file:
                                ...
                                filedata = filedata.replace('xxFrozenSwimmer', 'false')                                
                                ...
                                filedata = filedata.replace('xxNetworkSpringConstant', str(NetworkSpringConstant_i))
                            ...
                            process = subprocess.Popen(["make", "MPI_FIL"], stdout=subprocess.PIPE)
...
print("\nUse command:")
print("> `python batch_run_screen.py " + time + "" + '` to run.')
print("> `python batch_run_screen.py " + time + "" + ' 4` to run on 4 processors.')
print("> `python batch_run_screen.py " + time + "" + ' 4 restart` to run on 4 processors, forcing the use of initialisation files (`*.bak`, `*.par` and `*-springlinks.dat` from the output directory) and setting the timer at t=0.')
