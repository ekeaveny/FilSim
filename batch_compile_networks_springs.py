...
if SwimProblem:
    ...
    StepsPerPeriod = 1000 #600

    initialFrictionSteps = 0 # 2*StepsPerPeriod # 0  # 1000
    ...
    Nsw = [2700] # Maximum number of swimmers allowed (in spring node mode)
    TimeSteps = 22 * StepsPerPeriod  # 22  # Total timesteps
    ...
    NetworkSpringConstant = [0,0.01,0.1]  # k in the spring force k*(x-L)
...
print("\nUse command:")
print("* `python batch_run.py " + time + "" + '` to run.')
print("* `python batch_run.py " + time + "" + ' restart` to force the use of initialisation files (`*.bak`, `*.par` and `*-springlinks.dat` from the output directory) and setting the timer at t=0.')
